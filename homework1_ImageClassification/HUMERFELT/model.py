...
def show_images(images, labels, number = 25):
    for y in range(max(len(images) // number, 2)):
            plt.title("Pictures")
...
class Model:
    def __init__(self, path):
        ...
        #show_images(self.images, self.labels, number=25)
    ...
    def make_model(self, path):
        ...
        self.plot(value="accuracy", model=modellen)
        self.plot(value="loss", model=modellen)
        self.plot_confusion_matrix(self.test_labels, self.predict(self.test_images))
    
    def plot_confusion_matrix(self, y_test, pred, values_lbl = ['healthy', 'unhealthy']):
        conf = confusion_matrix(y_test,pred)
        plt.figure(figsize=(7,8))
        plt.imshow(conf, interpolation='nearest', cmap="Blues")
        #show the numbers in the boxes
        for x in range(len(conf)):
            for y in range(len(conf)):
                plt.annotate(str(conf[x][y]), xy=(y, x), 
    ...
    def plot(self, value, model):
        plt.plot(model.history[value])
        plt.plot(model.history['val_' + value])
